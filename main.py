...
def _preflight(env: dict, mode: str) -> tuple[list[_Check], bool]:
    ...
    if provider == "anthropic":
        ...
        checks.append(
            _Check(
                f"Ollama reachable at {root_url}",
                reachable,
                required=True,
...
def _show_preflight(checks: list[_Check], mode: str) -> None:
    ...
    console.print(
        Panel(
            table,
            title=f"[bold]Atlas — {mode} mode[/bold]",
            border_style="blue",
            expand=False,
        )
    )
...
def run(
    ...
    watch: bool = typer.Option(
        False, "--watch", "-w", help="Auto-restart when .py or config/.env files change"
    ),
) -> None:
    ...
    if not skip_checks:
        ...
        if not ok:
            ...
            if Confirm.ask("\nRun setup wizard now?", default=True):
                ...
                if not ok:
                    console.print(
                        "[red]Setup incomplete. Fix the issues above and try again.[/red]"
                    )
                    raise typer.Exit(1)
            else:
                raise typer.Exit(1)
            ...
            if warnings:
                console.print(
                    "[dim]Some optional features are unavailable (see ⚠ above)[/dim]"
                )
    ...
    if watch:
        console.print(
            "[dim]Watch mode — will restart on code or config changes.[/dim]\n"
        )
...
def logs(
    ...
    if not jsonl_files:
        ...
        console.print(
            "[dim]Logs appear after Atlas processes its first message.[/dim]\n"
        )
    else:
        console.print(
            f"Found [green]{len(jsonl_files)}[/green] log file(s) in [dim]{log_dir}[/dim]"
        )
...
def _run_setup() -> None:
    ...
    if provider == "anthropic":
        ...
        config["OPENROUTER_MODEL"] = Prompt.ask(
            "Model",
            default=existing.get("OPENROUTER_MODEL", "anthropic/claude-sonnet-4-5"),
        )
    elif provider == "ollama":
        config["OLLAMA_BASE_URL"] = Prompt.ask(
            "Ollama base URL",
            default=existing.get("OLLAMA_BASE_URL", "http://localhost:11434/v1"),
        )
    ...
    if Confirm.ask(
        "Configure Parakeet voice transcription?",
        default=bool(existing.get("PARAKEET_URL")),
    ):
        config["PARAKEET_URL"] = Prompt.ask(
            "Parakeet server URL",
            default=existing.get("PARAKEET_URL", "http://localhost:8765"),
        )
...
async def _run_with_heartbeat(brain, channel: BaseChannel) -> None:
    ...
    try:
        ...
        console.print(
            f"[bold green]✓ {agent_name} is running![/bold green] Press Ctrl+C to stop."
        )
