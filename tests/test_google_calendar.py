...
def patch_services(service_map: dict):
    ...
    return patch("skills.google_calendar.tool._get_services", side_effect=fake_get_services)
...
async def test_list_accounts(tmp_path):
    ...
    configs = [
        {"name": "personal", "creds_file": tmp_path / "creds_personal.json", "token_file": tmp_path / "tok.json"},
        {"name": "work",     "creds_file": tmp_path / "creds_work.json",     "token_file": tmp_path / "tok2.json"},
    ]
    (tmp_path / "creds_personal.json").write_text("{}")  # exists
    # creds_work.json does NOT exist → shows warning

    with patch("skills.google_calendar.tool._load_account_configs", return_value=configs):
...
async def test_create_event_requires_account_when_multi():
    ...
    with patch_services({"personal": svc1, "work": svc2}):
        result = await run(
            ...
            summary="Meeting", start="2026-03-20T10:00:00", end="2026-03-20T11:00:00",
        )
...
async def test_create_event_with_specific_account():
    ...
    with patch_services({"personal": svc}):
        result = await run(
            action="create_event", account="personal",
            summary="Dentist", start="2026-03-20T14:00:00", end="2026-03-20T15:00:00",
        )
...
async def test_update_event_success():
    ...
    with patch_services({"work": svc}):
        result = await run(action="update_event", account="work",
                           event_id="evt1", summary="New Title")
...
async def test_setup_account_file_not_found():
    from skills.google_calendar.tool import run
    result = await run(action="setup_account",
...
async def test_setup_account_success(tmp_path):
    ...
    with patch("skills.google_calendar.tool._CONFIG_DIR", config_dir), \
         patch("skills.google_calendar.tool._ACCOUNTS_FILE", config_dir / "google_accounts.json"), \
...
async def test_missing_credentials_returns_setup_message():
    ...
    with patch("skills.google_calendar.tool._get_services",
